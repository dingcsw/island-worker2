...
class OptionMoneyMarket extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedOption1: 'team',
      selectedOption2: 'amount',
    }
  ...
  handleCheck(state) {
    ...
    if (selectedOption2 === 'amount') return false;
    return true;
  }
    ...
    const showCheckResult = isShowCheckResult ? 
      <div className="row island-row-result">
        <div className="col-12 island-check-result">
          <table className="table">
            ...
            <tbody>
              <tr>
                <th scope="row">金錢</th>
                <td>{teams[selectedOption1]['money']} → {Number(teams[selectedOption1]['money']) - selectedOption2}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <button 
          ...
          onClick={() => {
            const newData = {
              money: Number(teams[selectedOption1]['money']) - selectedOption2
            }
...
export default OptionMoneyMarket; 
