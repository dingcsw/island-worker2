...
class OptionPurged extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleCheck(state) {
    const { selectedOption1 } = state;
    if (selectedOption1 === 'team') return false;
    return (this.props.teams[selectedOption1]['jail'] === 'in');
  }

  render() {
    const { selectedOption1, isShowCheckResult } = this.state;
    ...
    const showCheckResult = isShowCheckResult ? 
      <div className="row island-row-result">
        <div className="col-12 island-check-result">
          <table className="table">
            <thead>
              <tr>
                <th></th>
                <th>Team {selectedOption1} will be set free.</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <th scope="row">金錢</th>
                <td>{teams[selectedOption1]['money']} → {Number(teams[selectedOption1]['money']) - 1000}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <button 
          ...
          onClick={() => {
            const newData = {
              money: Number(teams[selectedOption1]['money']) - 1000,
              jail: "free"
            }
            this.props.action(newData, selectedOption1);
        ...
        <div className="row">
          <div className="input-group col-12">
            ...
            <select 
              ...
              id="formSelect1" 
              style={{ 'fontSize': '250%', 'padding': '2.5%', 'width': '40%', 'borderBottomLeftRadius': '0px' }}>
