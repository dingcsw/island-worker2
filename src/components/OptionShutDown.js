...
class OptionShutDown extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleCheck(state) {
    ...
    return this.props.teams[selectedOption2]['jail'] === 'needed';
  }
    ...
    const showCheckResult = isShowCheckResult ? 
      <div className="row island-row-result">
        <div className="col-12 island-check-result">
          <b>Team {selectedOption1} will shut down {selectedOption2}.</b>
        </div>
        <button 
          type="submit" 
          ...
          onClick={() => {
            const newData1 = {
              earned: Number(teams[selectedOption1]['earned']) + 1000,
              head: Number(teams[selectedOption1]['head'] + 1)
            }
            const newData2 = {
              jail: 'in'
            }
            if (this.props.env['jailTimestamp'] === 0) {
              const d = new Date();
              this.props.actionEnv({ jailTimestamp: d.getTime() });
            }
